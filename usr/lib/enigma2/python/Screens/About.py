...
class About(Screen):
    ...
    def __init__(self, session):
        ...
        AboutText += _('Build date: 2018-02-03') + '\n'
        AboutText += _('Devlopeur: Redouane') + '\n'
...
class CommitInfo(Screen):
    ...
    def __init__(self, session):
        ...
        self.projects = [
         ...
         ('https://api.github.com/repos/openpli/enigma2-plugin-extensions-xmltvimport/commits',
 'Plugin Xmltvimport'),
...
class Troubleshoot(Screen):
    ...
    def getLogFilesList(self):
        ...
        return [ x for x in sorted(glob.glob('/mnt/hdd/*.log'), key=lambda x: os.path.isfile(x) and os.path.getmtime(x))
               ] + (os.path.isfile(home_root) and [home_root] or []) + (os.path.isfile(tmp) and [tmp] or [])

    def updateOptions(self):
        self.titles = [
         'dmesg', 'ifconfig', 'df', 'top', 'ps']
