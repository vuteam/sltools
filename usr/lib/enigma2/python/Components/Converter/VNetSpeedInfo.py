...
#
# <widget source="session.CurrentService" render="Label" position="1655,775" size="95,30" font="Regular; 26" halign="center" foregroundColor="yellow" backgroundColor="header" transparent="1" zPosition="2" noWrap="1">
            # <convert type="VNetSpeedInfo">NET_TYP</convert>
   # </widget>
   # <widget source="session.CurrentService" render="Label" position="1684,775" size="246,56" font="Regular; 26" halign="center" foregroundColor="red" backgroundColor="header" transparent="1" zPosition="2" noWrap="1">
...
#		if ((flaglan == 1) and (flagwlan == 0)) or ((flaglan == 1) and (flagwlan == 1)):
		...
		if self.type == self.RCL:
			return "%2.3fKB/s" % self.lanreceive
		elif self.type == self.TML:
			return "%2.3fKB/s" % self.lantransmit
		elif self.type == self.RCW:
			return "%2.3fKB/s" % self.wlanreceive
		elif self.type == self.TMW:
			return "%2.3fKB/s" % self.wlantransmit
			...
			return "%2.3fKB/s" % self.lanreceivemb
		elif self.type == self.TML_MB:
			return "%2.3fKB/s" % self.lantransmitmb
		elif self.type == self.RCW_MB:
			return "%2.3fKB/s" % self.wlanreceivemb
		elif self.type == self.TMW_MB:
			return "%2.3fKB/s" % self.wlantransmitmb
		elif self.type == self.RC:
			return "%2.3fKB/s" % self.receive
		elif self.type == self.TM:
			return "%2.3fKB/s" % self.transmit
			...
			return "%2.3fKB/s" % self.receivemb
		elif self.type == self.TM_MB:
			return "%2.3fKB/s" % self.transmitmb
